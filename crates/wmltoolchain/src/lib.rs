...
use wmlvm::{Program as VmProgram, RunOutcome, WorkerId};
...
fn platform_mask(platform: PlatformProfile) -> u64 {
    match platform.kind {
        wmlplatform::PlatformKind::Native => 1 << 0,
        wmlplatform::PlatformKind::Wasm => 1 << 1,
        wmlplatform::PlatformKind::Egui => 1 << 2,
    }
}
