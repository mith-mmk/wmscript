use wmfrontend::{FrontendConfig, GuiFontPreset, launch_frontend_gui, run_frontend};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut config = FrontendConfig::new(PlatformProfile::native(), project);
    config.step_limit = 32;

    let report = run_frontend(config)?;
    launch_frontend_gui(report, GuiFontPreset::default_preset())?;
    Ok(())
}
