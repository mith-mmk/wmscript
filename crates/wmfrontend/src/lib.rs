...
impl FrontendApp {
    fn new(
        ...
        let mut runtime = Runtime::new(
            wmruntime::RuntimeConfig::new(config.platform).with_step_limit(config.step_limit),
        );
            ...
            Ok(execution) => {
                ...
                *self.log_lines.borrow_mut() = log_lines.clone();
                ...
                for asset in &self.config.project.assets {
                    if matches!(asset.resource_type, wmresource::ResourceType::Image) {
                        ctx.set_image(
...
fn to_ui_message_window_state(state: RuntimeMessageWindowState) -> wmui::UiMessageWindowState {
    wmui::UiMessageWindowState {
